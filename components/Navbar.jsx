...
const Navbar = () => {
  const token = useSelector(state => state?.vote?.currentVoter?.token)
    ...
    //func to close nav menu on small screens when link is clicked
    const closeNavMenu=() => {
  ...
  return (
    <nav>
      <div className="container nav_container">
        <Link to="/" className='nav_logo'>Online Voting</Link>
       <div>
        {token && showNav && <menu>
          <NavLink to="/elections" onClick={closeNavMenu}>Elections</NavLink>
          <NavLink to="/result" onClick={closeNavMenu}>Results</NavLink>
